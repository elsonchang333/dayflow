...
function renderTodoList() {
  ...
  const currentDate = Utils.formatDate(AppState.currentDate).full;
  const todayTodos = AppState.todos.filter(t => t.date === currentDate);
  
  if (todayTodos.length === 0) {
    container.innerHTML = '<div style="text-align:center;color:#9ca3af;padding:20px;">暂无待办事项</div>';
    return;
  }
...
function renderHabitList() {
  ...
  if (AppState.habits.length === 0) {
    container.innerHTML = '<div style="text-align:center;color:#9ca3af;padding:20px;">暂无习惯记录</div>';
    return;
  }
  
  container.innerHTML = AppState.habits.map(habit => {
    const isChecked = (habit.checkIns || []).includes(currentDate);
    return `
      <div class="habit-item">
        <button class="habit-check ${isChecked ? 'checked' : ''}" onclick="toggleHabit('${habit.id}')">
          ${isChecked ? '✓' : '○'}
        </button>
        <div class="habit-info">
          <span class="habit-name">${habit.icon || '✨'} ${escapeHtml(habit.name)}</span>
          <span class="habit-streak">连续 ${(habit.checkIns || []).length} 天</span>
        </div>
        <button class="todo-delete" onclick="deleteHabit('${habit.id}')">🗑️</button>
      </div>
    `;
  }).join('');
}
...
function toggleHabit(id) {
  ...
  const index = habit.checkIns.indexOf(currentDate);
  if (index > -1) {
    habit.checkIns.splice(index, 1);
  } else {
    habit.checkIns.push(currentDate);
  }
  ...
  if (AppState.currentDiaryId) {
    ...
    if (index > -1) {
      diary = {
        ...
        mood: AppState.selectedDiaryMood,
        updated_at: timestamp
      };
      AppState.diaries[index] = diary;
    }
  } else {
    // Create new
    diary = {
...
function renderDiaryList() {
  ...
  if (AppState.diaries.length === 0) {
    container.innerHTML = '<div style="text-align:center;color:#9ca3af;padding:40px;">还没有日记，写一篇吧！</div>';
    return;
  }
  
  container.innerHTML = AppState.diaries.map(diary => {
    const date = new Date(diary.date);
    return `
      <div class="diary-item" onclick="viewDiary('${diary.id}')">
        <div class="diary-header">
          <span class="diary-date">${date.getMonth() + 1}月${date.getDate()}日</span>
          <span class="diary-mood">${Utils.getMoodEmoji(diary.mood)}</span>
        </div>
...
function viewDiary(id) {
  ...
  AppState.selectedDiaryMood = diary.mood;
...
function selectDiaryMood(mood) {
  AppState.selectedDiaryMood = mood;
  document.querySelectorAll('.diary-mood-btn').forEach(btn => {
    btn.classList.remove('active');
  });
  const selectedBtn = document.querySelector(`.diary-mood-btn[data-mood="${mood}"]`);
...
function renderCalendar() {
  ...
  const date = AppState.currentDate || new Date();
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    ...
    const hasEvents = AppState.todos.some(t => t.date === dateStr) || 
                      AppState.diaries.some(d => d.date === dateStr);
    const isToday = dateStr === Utils.formatDate(new Date()).full;
    
    html += `<div class="calendar-day ${isToday ? 'today' : ''} ${hasEvents ? 'has-event' : ''}" onclick="selectCalendarDate('${dateStr}')">${day}</div>`;
  }
